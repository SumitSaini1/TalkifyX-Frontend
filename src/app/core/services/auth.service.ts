...
import {
  ...
  ApiResponse, ProfileUpdateRequest, ChangePasswordRequest, UserStatus
} from '../models';
...
export class AuthService {
  ...
  updateProfile(req: ProfileUpdateRequest): Observable<ApiResponse> {
    return this.http.put<ApiResponse>(`${this.base}/profile`, req).pipe(
  ...
  searchUsers(username: string): Observable<User[]> {
    return this.http.get<ApiResponse<User[]>>(`${this.base}/search`, { params: { username } }).pipe(
      map(r => r.data)
    );
  }
