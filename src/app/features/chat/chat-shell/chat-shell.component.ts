      ...
      <aside class="sidebar glass" [class.mobile-hidden]="showMobileChat()">
          ...
          <div class="brand">
            <div class="brand-icon">
              <svg viewBox="0 0 32 32" fill="none">
                ...
                <path
                  d="M6 10.5c0-1.5 1.2-2.7 2.7-2.7H23.3C24.8 7.8 26 9 26 10.5V18c0 1.5-1.2 2.7-2.7 2.7H18l-4 2.8v-2.8H8.7C7.2 20.7 6 19.5 6 18V10.5z"
                  fill="white"
                />
                <defs>
                  <linearGradient
        ...
        <div class="user-info" [routerLink]="['/profile']">
          <div class="avatar-wrap">
            ...
            <span
              class="status-dot"
              [class]="'status-' + (me()?.status || 'ONLINE').toLowerCase()"
            ></span>
          </div>
          <div class="user-text">
          ...
          @if (loading()) {
            ...
            <div class="empty-state">
              <svg viewBox="0 0 64 64" fill="none">
                <circle cx="32" cy="32" r="28" fill="#F3F0FF" />
                <path
                  d="M20 26c0-1.5 1.2-2.7 2.7-2.7H41.3C42.8 23.3 44 24.5 44 26V36c0 1.5-1.2 2.7-2.7 2.7H36l-5 3.5v-3.5H22.7C21.2 38.7 20 37.5 20 36V26z"
                  fill="#7C3AED"
                  opacity=".3"
                />
              </svg>
              ...
              >
                <div class="avatar-wrap">
                  ...
                  @if (room.type === "DM" && room.otherUser) {
                    <span
                      class="status-dot"
                      [class]="'status-' + getPresenceStatus(room.otherUser.id)"
                    ></span>
                  }
                </div>
                <div class="room-info">
            ...
            <div class="no-chat-content">
              <div class="no-chat-icon">
                <svg viewBox="0 0 80 80" fill="none">
                  ...
                  <path
                    d="M18 28c0-3 2.4-5.5 5.5-5.5H56.5C59.6 22.5 62 25 62 28V46c0 3-2.4 5.5-5.5 5.5H46l-8 6V51.5H23.5C20.4 51.5 18 49 18 46V28z"
                    fill="#7C3AED"
                    opacity=".15"
                  />
                  <path
                    d="M22 32c0-2.2 1.8-4 4-4H54c2.2 0 4 1.8 4 4V44c0 2.2-1.8 4-4 4H46l-6 4.5V48H26c-2.2 0-4-1.8-4-4V32z"
                    fill="#7C3AED"
                    opacity=".3"
                  />
                  <circle cx="33" cy="38" r="2" fill="#7C3AED" />
                  <circle cx="40" cy="38" r="2" fill="#7C3AED" />
                  <circle cx="47" cy="38" r="2" fill="#7C3AED" />
                </svg>
              </div>
...
export class ChatShellComponent implements OnInit, OnDestroy {
  ...
  private connectPresence(userId: number): void {
    ...
    this.presenceService
      .connect({ userId, status: "ONLINE", deviceType: "WEB", sessionId })
      .subscribe({
        next: () => {
          this.presencePingInterval = setInterval(() => {
            if (this.presenceSessionId) {
              this.presenceService
                .ping(this.presenceSessionId)
                .subscribe({ error: () => {} });
            }
          }, 30_000);
        },
  ...
  private disconnectPresence(): void {
    ...
    if (this.presenceSessionId) {
      this.presenceService
        .disconnect(this.presenceSessionId)
        .subscribe({ error: () => {} });
      ...
      .subscribe({
        next: (rooms) => {
              ...
              next: (msg) => {
                if (msg) {
                  this.rooms.update((list) =>
                    list.map((x) =>
                      x.roomId === r.roomId
                        ? { ...x, lastMessage: msg, lastMessageAt: msg.sentAt }
                        : x,
                    ),
                  );
                }
          ...
          rooms.forEach((r) => {
            this.roomService.getMembers(r.roomId).subscribe({
              next: (members) => {
                ...
                this.roomService
                  .getUnreadCountByDate(r.roomId, after)
                  .subscribe({
                    next: (count) => {
                      this.rooms.update((list) =>
                        list.map((x) =>
          ...
          const dmUserIds = rooms
            .filter((r) => r.type === "DM" && r.otherUser)
  ...
  getRoomName(room: Room): string {
    if (room.type === "DM" && room.otherUser)
      return room.otherUser.fullName || room.otherUser.username;
    return room.name;
  }
  ...
  getRoomAvatar(room: Room): string {
    ...
    if (room.type === "DM" && room.otherUser)
      return room.otherUser.avatarUrl || this.avatarPlaceholder(room.otherUser);
